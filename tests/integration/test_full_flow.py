...
from smart_mcp_proxy.models.schemas import EmbedderType, ToolMetadata
...
class TestFullIndexingFlow:
    ...
    async def test_incremental_indexing(self, temp_indexer_facade):
        ...
        server1_tools = await temp_indexer_facade.persistence.get_tools_by_server("server1")
        server2_tools = await temp_indexer_facade.persistence.get_tools_by_server("server2")
    ...
    async def test_search_ranking_quality(self, temp_indexer_facade):
        ...
        tools = [
            ("perfect_match", "Create virtual machine instance", "server1", ["vm", "create"]),
    ...
    async def test_large_scale_indexing(self, temp_indexer_facade):
        ...
        for i in range(50):
            ...
            category = ["compute", "storage", "network", "monitoring", "security"][i % 5]
        ...
        for query, expected_category in search_tests:
            ...
            category_matches = sum(1 for r in results 
                                 if expected_category in r.tool.name)
    ...
    async def test_multi_server_search_isolation(self, temp_indexer_facade):
        ...
        for server_name, expected_tools in servers_and_tools:
            server_tools = await temp_indexer_facade.persistence.get_tools_by_server(server_name)
