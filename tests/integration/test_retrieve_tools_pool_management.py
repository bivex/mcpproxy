...
class TestRetrieveToolsPoolManagement:
    ...
    def create_mock_search_result(self, tool_name: str, server_name: str, score: float) -> SearchResult:
    ...
    @pytest.mark.asyncio
    async def test_retrieve_tools_with_pool_limit_enforcement(self, mock_server_with_indexer):
        ...
        server.tool_pool_metadata = {
            "server1_old_tool1": {"timestamp": current_time - 1800, "score": 0.3},  # Old, low score
            "server1_old_tool2": {"timestamp": current_time - 900, "score": 0.8},   # Medium age, high score  
            "server2_old_tool3": {"timestamp": current_time, "score": 0.6}          # Fresh, medium score
        }
                ...
                if tool_name in server.current_tool_registrations:
                    ...
                    if tool_name in server.tool_pool_metadata:
                        ...
                        server.tool_pool_metadata[tool_name]["score"] = max(
                            server.tool_pool_metadata[tool_name]["score"], 
                            result.score
                        )
                    continue
    ...
    @pytest.mark.asyncio
    async def test_retrieve_tools_freshness_update_existing_tools(self, mock_server_with_indexer):
    ...
    @pytest.mark.asyncio
    async def test_retrieve_tools_no_eviction_when_under_limit(self, mock_server_with_indexer):
        ...
        search_results = [
            self.create_mock_search_result("new_tool", "server1", 0.7)
        ]
        server.indexer.search_tools = AsyncMock(return_value=search_results)
