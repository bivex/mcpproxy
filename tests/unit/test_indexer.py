...
from smart_mcp_proxy.indexer.bm25 import BM25Embedder
from smart_mcp_proxy.indexer.base import BaseEmbedder
from smart_mcp_proxy.models.schemas import EmbedderType, ToolMetadata, SearchResult
from smart_mcp_proxy.utils.hashing import compute_tool_hash
...
class TestIndexerFacade:
    ...
    def test_indexer_facade_embedder_creation(self, mock_persistence, temp_index_dir):
        ...
        # Test BM25
        indexer_bm25 = IndexerFacade(mock_persistence, EmbedderType.BM25, index_dir=temp_index_dir)
    ...
    async def test_index_tool_duplicate_hash(self, mock_persistence, temp_index_dir):
        ...
        existing_tool = ToolMetadata(
            ...
            hash="existing_hash", server_name="server"
        )
    ...
    async def test_reindex_all_tools(self, mock_persistence, temp_index_dir):
        ...
        sample_tools = [
            ToolMetadata(
                id=1, name="create_instance", description="Create VM",
                hash="hash1", server_name="api", params_json='{"parameters": {}, "tags": ["compute"], "annotations": null}'
            ),
            ToolMetadata(
                id=2, name="delete_volume", description="Delete storage",
                hash="hash2", server_name="api", params_json='{"parameters": {}, "tags": ["storage"], "annotations": null}'
            )
        ]
    ...
    async def test_search_tools_bm25(self, mock_persistence, temp_index_dir):
        ...
        sample_tools = [
            ToolMetadata(
                id=1, name="create_instance", description="Create VM",
                hash="hash1", server_name="api", params_json='{"parameters": {}, "tags": [], "annotations": null}'
            ),
            ToolMetadata(
                id=2, name="delete_volume", description="Delete storage",
                hash="hash2", server_name="api", params_json='{"parameters": {}, "tags": [], "annotations": null}'
            )
        ]
        mock_persistence.get_all_tools.return_value = sample_tools
    ...
    async def test_search_tools_vector_embedder(self, mock_persistence, temp_index_dir):
        ...
        mock_results = [
            SearchResult(
                tool=ToolMetadata(
                    id=1, name="test_tool", description="desc",
                    hash="hash", server_name="server"
                ),
                score=0.9
            )
        ]
    ...
    async def test_index_multiple_tools_different_servers(self, temp_indexer_facade):
        ...
        server_a_tools = await temp_indexer_facade.persistence.get_tools_by_server("server-a")
        server_b_tools = await temp_indexer_facade.persistence.get_tools_by_server("server-b")
    ...
    async def test_index_and_search_with_tags(self, temp_indexer_facade):
        ...
        results = await temp_indexer_facade.search_tools("virtual machine creation", k=2)
        ...
        if len(results) > 1:
            ...
            assert (results[0].tool.name == "vm_creator") or (results[0].score >= results[1].score)
        else:
