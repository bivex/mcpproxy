...
from smart_mcp_proxy.persistence.faiss_store import FaissStore
...
class TestFaissStore:
    ...
    def mock_faiss(self):
        ...
        with patch('smart_mcp_proxy.persistence.faiss_store.faiss') as mock_faiss:
    ...
    async def test_faiss_store_initialization(self, mock_faiss):
        ...
        index_path = os.path.join(temp_dir, f"test_faiss_{os.getpid()}_{id(object())}.faiss")
...
class TestPersistenceFacade:
    ...
    @pytest.mark.asyncio
    async def test_store_tool_with_vector(self, temp_persistence_facade, sample_tool_metadata):
    ...
    @pytest.mark.asyncio
    async def test_get_tool_by_hash_facade(self, temp_persistence_facade, sample_tool_metadata):
        ...
        # Retrieve by hash
        retrieved_tool = await temp_persistence_facade.get_tool_by_hash(sample_tool_metadata.hash)
    ...
    @pytest.mark.asyncio
    async def test_update_tool_with_vector(self, temp_persistence_facade, sample_tool_metadata):
        ...
        # Store initial tool
        await temp_persistence_facade.store_tool_with_vector(sample_tool_metadata, vector1)
        
        # Update with new vector
        sample_tool_metadata.description = "Updated description"
        await temp_persistence_facade.update_tool_with_vector(sample_tool_metadata, vector2)
        
        # Verify update
        updated_tool = await temp_persistence_facade.get_tool_by_hash(sample_tool_metadata.hash)
        assert updated_tool.description == "Updated description"
    
    @pytest.mark.asyncio
    async def test_search_similar_tools(self, temp_persistence_facade, sample_tool_metadata_list):
        ...
        with patch.object(temp_persistence_facade.vector_store, 'search') as mock_search:
            # Mock search to return first tool as most similar
            mock_search.return_value = (np.array([0.1, 0.5, 0.8]), np.array([1, 2, 3]))
            
            results = await temp_persistence_facade.search_similar_tools(query_vector, k=3)
    ...
    @pytest.mark.asyncio
    async def test_get_all_tools_facade(self, temp_persistence_facade, sample_tool_metadata_list):
    ...
    @pytest.mark.asyncio
    async def test_get_tools_by_server_facade(self, temp_persistence_facade, sample_tool_metadata_list):
    ...
    @pytest.mark.asyncio
    async def test_delete_tools_by_server_facade(self, temp_persistence_facade, sample_tool_metadata_list):
    ...
    @pytest.mark.asyncio
    async def test_get_vector_count_facade(self, temp_persistence_facade, sample_tool_metadata_list):
