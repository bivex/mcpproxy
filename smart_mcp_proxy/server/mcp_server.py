...
from ..models.schemas import ProxyConfig, ServerConfig, ToolRegistration, SearchResult
...
class SmartMCPProxyServer:
    ...
    def run(self) -> None:
        ...
        try:
            logger.info("Starting Smart MCP Proxy...")
            
            # Run the FastMCP app (this will handle lifespan automatically)
            self.mcp.run()
    ...
    async def _register_proxy_tool(self, tool_metadata: Any, tool_name: str) -> None:
        ...
        async def proxy_tool(**kwargs) -> str:
            ...
            try:
                ...
                return f"Error calling {original_tool_name}: {str(e)}"
        
        # Track this registration
        self.current_tool_registrations[tool_name] = proxy_tool
