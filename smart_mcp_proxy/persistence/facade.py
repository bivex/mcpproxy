...
from ..models.schemas import ToolMetadata, SearchResult


class PersistenceFacade:
    """Unified facade for database and vector store operations."""
    
    def __init__(self, db_path: str = "proxy.db", index_path: str = "tools.faiss", 
                 vector_dimension: int = 384):
        self.db = DatabaseManager(db_path)
        self.vector_store = FaissStore(index_path, vector_dimension)
    
    async def store_tool_with_vector(self, tool: ToolMetadata, vector: np.ndarray) -> int:
    ...
    async def update_tool_with_vector(self, tool: ToolMetadata, vector: np.ndarray) -> None:
    ...
    async def search_similar_tools(self, query_vector: np.ndarray, k: int = 5) -> list[SearchResult]:
