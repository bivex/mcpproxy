...
class DatabaseManager:
    ...
    async def insert_tool(self, tool: ToolMetadata) -> int:
        ...
        async with self.get_connection() as conn:
            cursor = conn.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?)
            """, (tool.name, tool.description, tool.hash, tool.server_name, 
                  tool.faiss_vector_id, tool.params_json))
            conn.commit()
    ...
    async def update_tool(self, tool: ToolMetadata) -> None:
        ...
        async with self.get_connection() as conn:
            conn.execute("""
                ...
                WHERE id=?
            """, (tool.description, tool.hash, tool.faiss_vector_id, 
                  tool.params_json, tool.id))
            conn.commit()
    ...
    async def get_tools_by_server(self, server_name: str) -> list[ToolMetadata]:
        ...
        async with self.get_connection() as conn:
            rows = conn.execute("SELECT * FROM tools WHERE server_name=?", (server_name,)).fetchall()
            return [ToolMetadata(**dict(row)) for row in rows]
