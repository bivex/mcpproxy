...
class ServerDiscoveryManager:
    ...
    async def _create_url_client_proxy(self, server_name: str, url: str):
        ...
        try:
            logger.debug(
                f"Creating URL-based client for {server_name} at {url}"
            )
            client = Client(url)
            ...
            return client, proxy_server
        except Exception as e:
            logger.error(f"Failed to create URL client/proxy for {server_name}: {type(e).__name__}: {e}")
    ...
    async def _create_cmd_client_proxy(self, server_name: str, command: str, args: list, env: dict):
        ...
        try:
            ...
            logger.error(f"Failed to create command client/proxy for {server_name}: {type(e).__name__}: {e}")
            logger.error(f"Command client/proxy error details for {server_name}:", exc_info=True)
            return None, None
    ...
    async def _discover_server_tools(
        ...
        try:
            tools = await self._get_tools_from_proxy(server_name, proxy_server)
            if not tools:
                return {}

            tool_names = {}
            for tool_name, tool_obj in tools.items():
                sanitized_key = await self._process_and_index_tool(server_name, tool_name, tool_obj)
                if sanitized_key:
                    tool_names[tool_name] = sanitized_key

            logger.info(
                f"Successfully indexed {len(tool_names)}/{len(tools)} tools from {server_name}"
            )
            return tool_names
    ...
    async def _get_tools_from_proxy(self, server_name: str, proxy_server: FastMCP) -> dict:
        logger = get_logger()
    ...
    async def _process_and_index_tool(self, server_name: str, tool_name: str, tool_obj) -> str | None:
        logger = get_logger()
        try:
            if self.tool_pool_manager:
                sanitized_key = sanitize_tool_name(server_name, tool_name, self.config.tool_name_limit)
            ...
            if (
                ...
                logger.error("Vector dimension mismatch for tool {tool_name}. This usually means:")
                logger.error("1. Existing FAISS index has different dimension than current embedder")
                logger.error("2. Solution: Set MCPPROXY_RESET_DATA=true environment variable to reset data")
